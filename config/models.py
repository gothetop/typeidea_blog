...
class Link(models.Model):
    ...
    status = models.SmallIntegerField(default=STATUS_NORMAL,
    ...
    @classmethod
    def get_links(cls):
        queryset = cls.objects.filter(status=Link.STATUS_NORMAL)
        return queryset
...
class SiderBar(models.Model):
    ...
    def content_html(self):
        ...
        if self.display_type == self.DISPLAY_HTML:
            ...
            new_posts = posts.order_by("-created_time")[:3]   # 取最近的2篇文章
            context = {
                "posts": new_posts
            }
